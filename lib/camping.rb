...
:headers,:body,:url_prefix;T={};L=:layout;def lookup n;T.fetch(n.to_sym){|k|t=Views.
...
@cookies._n.each{|k,v|r.set_cookie k,v};r.to_a end;def initialize env,m,p
r=@request=Rack:: Request.new(@env=env);@root,@input,@cookies,@state,@headers,
@status,@method,@url_prefix=r.script_name.sub(/\/$/,''),n(r.params),Cookies[r.cookies],
H[r.session[SK]||{}],{E=>Z},m=~/r(\d+)/?$1.to_i: 200,m,p;@cookies._p=self/"/" end
...
A= ->(c,u,p){d=p.dup;d.chop! if u=='';u.prepend("/"+d) if !["I","Index"].include? c.to_s
if c.to_s=="Index";while d[-1] == "/"; d.chop! end;u.prepend("/"+d)end;u}
N=H.new{|_,x|x.downcase}.merge!("N"=>'(\d+)',"X"=>'([^/]+)',"Index"=>'');def M(p);
def M(p);end;constants.map{|c|k=const_get(c);k.send:include,C,X,Base,Helpers,Models
@r=[k]+@r if @r-[k]==@r;k.meta_def(:urls){[A.(k,"#{c.to_s.scan(/.[^A-Z]*/)
.map(&N.method(:[]))*'/'}",p)]}if !k.respond_to?:urls}end end;I=R()end;X=
Controllers;class<<self;def routes;X.M prx;(Apps.map(&:routes)<<X.v).flatten;end;
def prx;@_prx ||= PRF.(O[:url_prefix])end;PRF= ->(p){f = p.dup;return "" if f=="";
f.chop!until f[-1]!="/";f.slice!(0)until f[0]!="/";f<<"/"}
...
env_for('',h.delete(:env)||{})];k=X.const_get(c).new(e,m.to_s,prx);h.each{|i,v|k.
send"#{i}=",v};k.service(*a) end;def use*a,&b;m=a.shift.new(method(:call),*a,&b)
meta_def(:call){|e|m.call(e)}end;def pack*a,&b;G<<g=a.shift;include g;g.setup(self,*a,&b)end
